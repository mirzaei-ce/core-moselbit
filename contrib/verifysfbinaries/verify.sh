...
###   This script attempts to download the signature file SHA256SUMS.asc from moselbit.org
...
WORKINGDIR="/tmp/moselbit"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test/"
BASEDIR="https://moselbit.org/bin/"
VERSIONPREFIX="moselbit-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'moselbit-',
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Moselbit Core binary release signing key installed?"
   fi
