...
{
    std::cout <<
        "This program outputs Moselbit addresses and private keys from a wallet.dat file" << std::endl
        << std::endl
        << "Usage and options: "
        << std::endl
...
std::string WalletUtilityDB::getAddress(CDataStream ssKey)
{
    ...
    std::string strAddr = CMoselbitAddress(id).ToString();
...
std::string WalletUtilityDB::getKey(CDataStream ssKey, CDataStream ssValue)
{
    ...
    if (key.Load(pkey, vchPubKey, true))
        strKey = CMoselbitSecret(key).ToString();
...
std::string WalletUtilityDB::getCryptedKey(CDataStream ssKey, CDataStream ssValue, std::string masterPass)
{
    ...
    std::string strKey = CMoselbitSecret(key).ToString();
    return strKey;
}
